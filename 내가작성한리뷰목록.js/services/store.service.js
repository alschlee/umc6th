...
// 내가 진행 중인 미션 목록 조회
export const getOngoingMissions = async (userId, page) => {
  const limit = 10; // 페이지당 목록 수
  const offset = (page - 1) * limit;

  try {
    // 임시로 하드코딩된 값
    const dummyMissions = [
      { missionId: 1, storeName: "반이학생마라탕", price: 5000, points: 100, status: "진행중" },
      { missionId: 2, storeName: "반이학생마라반", price: 10000, points: 200, status: "진행중" },
      { missionId: 3, storeName: "반이학생치킨집", price: 15000, points: 300, status: "진행중" }
    ]; // 실제 DB 조회 로직으로 변경 필요
    return dummyMissions;
  } catch (error) {
    throw new BaseError(status.INTERNAL_SERVER_ERROR, "진행중인 미션 목록 조회 실패");
  }
};